...
describe('Edit Component - Cursor Preservation', () => {
    ...
    it('should preserve cursor position after re-rendering - cursor at the beginning', async () => {
        ...
        await vi.waitUntil(() => editorArea.textContent === initialContent); // Wait for content to render

        editorArea.focus();
        editorArea.setSelectionRange(0, 0); // Set cursor at the beginning
        const initialCursorPosition = editorArea.selectionStart;

        editInstance.renderContent(); // Trigger re-render
        await vi.waitUntil(() => editorArea.textContent === initialContent); // Wait for re-render
    ...
    it('should preserve cursor position after re-rendering - cursor in the middle', async () => {
        ...
        editorArea.focus();
        const cursorPosition = 10; // Position in the middle
    ...
    it('should preserve cursor position after re-rendering - cursor at the end', async () => {
        ...
        editorArea.focus();
        const cursorPosition = initialContent.length; // Position at the end
        editorArea.setSelectionRange(cursorPosition, cursorPosition);
        const initialCursorPosition = editorArea.selectionStart;
    ...
    it('should preserve cursor position after re-rendering - with tags', async () => {
        ...
        yDoc.getText('content').insert(0, initialContent);
        await vi.waitUntil(() => editorArea.textContent.includes('Text before')); // Wait for content with tag to render

        editorArea.focus();
        const cursorPosition = 15; // Position in the middle of text before tag
        editorArea.setSelectionRange(cursorPosition, cursorPosition);
        const initialCursorPosition = editorArea.selectionStart;

        editInstance.renderContent();
        await vi.waitUntil(() => editorArea.textContent.includes('Text before')); // Wait for re-render
    ...
    it('should preserve cursor position after content is changed programmatically', async () => {
        ...
        await vi.waitUntil(() => editorArea.textContent === newContent); // Wait for content to update

        expect(editorArea.selectionStart).toBe(initialCursorPosition); // Cursor should still be at the same *relative* position if possible, or at the end if the content is shorter
    });
    ...
    it('should place cursor at the end if initial cursor position is out of bounds after re-render', async () => {
        ...
        expect(editorArea.selectionStart).toBe(shorterContent.length); // Cursor should be moved to the end
    });
});
